...
if(process.platform === 'darwin') {
  ...
  const get = () => osascript('output volume of (get volume settings)').then(volume => Number.parseInt(volume, 10));
  const isMuted = () => osascript('output muted of (get volume settings)').then(stdout => (`${stdout}`.trim() === 'true'));
  const getInfo = () => Promise.all([get(), isMuted()])
    .then(results => ({
      volume : results[0],
      isMuted: results[1]
    }));
  ...
  module.exports.set = rawValue => input(rawValue).then(value => osascript(`set volume output volume ${value}`)).then(_result => getInfo());  // 負数や100超でもエラーにはならない
  module.exports.isMuted = isMuted;
  module.exports.mute = () => osascript('set volume with output muted').then(_result => getInfo());
  module.exports.unmute = () => osascript('set volume without output muted').then(_result => getInfo());
}
  ...
  const executablePath = path.join(__dirname, 'loudness-win32.exe');  // adjust_get_current_system_volume_vista_plus.exe
